...
|----|----|----|
|SleepInterval|秒|循环间隔|
|FMZ_AccessKey|注意是FMZ平台的AccessKey，不是交易所的KEY|FMZ平台的AccessKey|
|FMZ_SecretKey|注意是FMZ平台的SecretKey，不是交易所的KEY|FMZ平台的SecretKey|
|maxBuffSignalRowDisplay|状态栏中最大的信号显示行数，设置20就显示最近20条信号记录|最大信号显示行数|
|isLogReset|勾选重置|重置所有日志|
...
|----|----|----|
|TestSignal|仅仅用于模拟TradingView发送的webhook请求|测试信号|
|evalCode|直接执行Javascript代码，可以用于测试、切换模拟盘等操作|执行Javascript代码|
